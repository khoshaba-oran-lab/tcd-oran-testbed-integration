...
RUNTIME_GATE_ALLOW_DEVELOPMENT_FILES_UNTRACKED=""

FINAL_CHECKPOINT_REQUIRE_REMOTE_SYNC=""
FINAL_CHECKPOINT_REQUIRE_DEVELOPMENT_SCOPE_TRACKED=""
FINAL_CHECKPOINT_REQUIRE_DEVELOPMENT_SCOPE_CLEAN=""
...
if [ "$REPOSITORY_POLICY_GATE" = "PASS" ]; then
    ...
    REPOSITORY_DEVELOPMENT_FILE_COUNT="$(
        get_value \
            "$REPOSITORY_POLICY" \
            DEVELOPMENT_FILE_COUNT
    )"
fi

echo "REPOSITORY_POLICY_VERSION=${REPOSITORY_POLICY_VERSION}"
echo "REPOSITORY_PROFILE=${REPOSITORY_PROFILE}"
...
if [ "$REPOSITORY_POLICY_VERSION" = "2" ] &&
   [ -n "$REPOSITORY_PROFILE" ] &&
   [ -n "$REPOSITORY_EXPECTED_BRANCH" ] &&
   ...
   [ "$RUNTIME_GATE_ALLOW_DEVELOPMENT_FILES_UNTRACKED" = "true" ] &&
   [ "$FINAL_CHECKPOINT_REQUIRE_REMOTE_SYNC" = "true" ] &&
   [ "$FINAL_CHECKPOINT_REQUIRE_DEVELOPMENT_SCOPE_TRACKED" = "true" ] &&
   [ "$FINAL_CHECKPOINT_REQUIRE_DEVELOPMENT_SCOPE_CLEAN" = "true" ] &&
   [ "$FINAL_CHECKPOINT_REQUIRE_GLOBAL_CLEAN_WORKTREE" = "false" ] &&
   [[ "$REPOSITORY_TRACKED_DEPENDENCY_COUNT" =~ ^[0-9]+$ ]] &&
   [ "$REPOSITORY_TRACKED_DEPENDENCY_COUNT" -gt 0 ] &&
   [[ "$REPOSITORY_DEVELOPMENT_FILE_COUNT" =~ ^[0-9]+$ ]] &&
   [ "$REPOSITORY_DEVELOPMENT_FILE_COUNT" -gt 0 ]
then
...
echo "REPOSITORY_DEVELOPMENT_UNTRACKED_FILES_ALLOWED=YES"
...
echo
echo "===== GLOBAL PROCESS CONTINUITY: START FINGERPRINT ====="

PROCESS_CONTINUITY_START_CAPTURE_GATE="FAIL"
PROCESS_CONTINUITY_END_CAPTURE_GATE="NOT_EVALUATED"
PROCESS_CONTINUITY_FINGERPRINT_MATCH_GATE="NOT_EVALUATED"
PROCESS_CONTINUITY_GATE="FAIL"

sci_oran_capture_global_runtime_fingerprint()
{
    local compose_map
    local core
    local gnb
    ...
    resolve_fp_service()
    {
        ...
        if [ "$count" -ne 1 ]; then
            printf 'ERROR=SERVICE_RESOLUTION:%s:%s\n' \
                "$service" \
                "$count"
            return 0
        fi
    ...
    core="$(resolve_fp_service 5gc)"
    gnb="$(resolve_fp_service gnb)"
    ue="$(resolve_fp_service srsue)"
    ric="$RIC_NAME"
    ...
    if ! docker inspect "$ric" >/dev/null 2>&1; then
        printf 'ERROR=RIC_RESOLUTION:%s\n' "$ric"
        return 0
    fi
    ...
    capture_fp_role()
    {
        ...
        docker inspect \
            "$container" \
            --format \
'ROLE='"$role"'|NAME={{.Name}}|ID={{.Id}}|PID={{.State.Pid}}|STARTED_AT={{.State.StartedAt}}|RESTART_COUNT={{.RestartCount}}|IMAGE_ID={{.Image}}|RUNNING={{.State.Running}}|STATUS={{.State.Status}}|HEALTH={{if index .State "Health"}}{{(index .State "Health").Status}}{{else}}none{{end}}' \
            2>/dev/null |
        sed 's/|NAME=\//|NAME=/'
    }

    capture_fp_role "5GC" "$core"
    capture_fp_role "RIC" "$ric"
    capture_fp_role "GNB" "$gnb"
    capture_fp_role "UE" "$ue"
}

PROCESS_CONTINUITY_START_FINGERPRINT="$(
    sci_oran_capture_global_runtime_fingerprint
)"

printf '%s\n' "$PROCESS_CONTINUITY_START_FINGERPRINT"

PROCESS_CONTINUITY_START_ROLE_COUNT="$(
    printf '%s\n' "$PROCESS_CONTINUITY_START_FINGERPRINT" |
    grep -c '^ROLE=' ||
    true
)"

PROCESS_CONTINUITY_START_ERROR_COUNT="$(
    printf '%s\n' "$PROCESS_CONTINUITY_START_FINGERPRINT" |
    grep -c '^ERROR=' ||
    true
)"

if [ "$PROCESS_CONTINUITY_START_ROLE_COUNT" -eq 4 ] &&
   [ "$PROCESS_CONTINUITY_START_ERROR_COUNT" -eq 0 ]
then
    PROCESS_CONTINUITY_START_CAPTURE_GATE="PASS"
fi

PROCESS_CONTINUITY_START_SHA256="$(
    printf '%s\n' "$PROCESS_CONTINUITY_START_FINGERPRINT" |
    sha256sum |
    awk '{print $1}'
)"

echo "PROCESS_CONTINUITY_START_ROLE_COUNT=${PROCESS_CONTINUITY_START_ROLE_COUNT}"
echo "PROCESS_CONTINUITY_START_ERROR_COUNT=${PROCESS_CONTINUITY_START_ERROR_COUNT}"
echo "PROCESS_CONTINUITY_START_SHA256=${PROCESS_CONTINUITY_START_SHA256}"
echo "PROCESS_CONTINUITY_START_CAPTURE_GATE=${PROCESS_CONTINUITY_START_CAPTURE_GATE}"
...
if [ "$E2SM_RC_READINESS_GATE" = "PASS" ]; then
    RAN_FUNCTION_3_GATE="PASS"
else
    RAN_FUNCTION_3_GATE="FAIL"
fi

echo "RAN_FUNCTION_3_GATE=${RAN_FUNCTION_3_GATE}"
...
USER_PLANE_EVIDENCE_TIMESTAMP="MISSING"
USER_PLANE_EVIDENCE_AGE_SECONDS="MISSING"
...
echo
echo "===== GLOBAL PROCESS CONTINUITY: END FINGERPRINT ====="

PROCESS_CONTINUITY_END_FINGERPRINT="$(
    sci_oran_capture_global_runtime_fingerprint
)"

printf '%s\n' "$PROCESS_CONTINUITY_END_FINGERPRINT"

PROCESS_CONTINUITY_END_ROLE_COUNT="$(
    printf '%s\n' "$PROCESS_CONTINUITY_END_FINGERPRINT" |
    grep -c '^ROLE=' ||
    true
)"

PROCESS_CONTINUITY_END_ERROR_COUNT="$(
    printf '%s\n' "$PROCESS_CONTINUITY_END_FINGERPRINT" |
    grep -c '^ERROR=' ||
    true
)"

if [ "$PROCESS_CONTINUITY_END_ROLE_COUNT" -eq 4 ] &&
   [ "$PROCESS_CONTINUITY_END_ERROR_COUNT" -eq 0 ]
then
    PROCESS_CONTINUITY_END_CAPTURE_GATE="PASS"
else
    PROCESS_CONTINUITY_END_CAPTURE_GATE="FAIL"
fi

PROCESS_CONTINUITY_END_SHA256="$(
    printf '%s\n' "$PROCESS_CONTINUITY_END_FINGERPRINT" |
    sha256sum |
    awk '{print $1}'
)"

echo "PROCESS_CONTINUITY_END_ROLE_COUNT=${PROCESS_CONTINUITY_END_ROLE_COUNT}"
echo "PROCESS_CONTINUITY_END_ERROR_COUNT=${PROCESS_CONTINUITY_END_ERROR_COUNT}"
echo "PROCESS_CONTINUITY_END_SHA256=${PROCESS_CONTINUITY_END_SHA256}"
echo "PROCESS_CONTINUITY_END_CAPTURE_GATE=${PROCESS_CONTINUITY_END_CAPTURE_GATE}"

echo
echo "----- GLOBAL FINGERPRINT COMPARISON -----"

if [ "$PROCESS_CONTINUITY_START_CAPTURE_GATE" = "PASS" ] &&
   [ "$PROCESS_CONTINUITY_END_CAPTURE_GATE" = "PASS" ] &&
   [ "$PROCESS_CONTINUITY_START_FINGERPRINT" = "$PROCESS_CONTINUITY_END_FINGERPRINT" ] &&
   [ "$PROCESS_CONTINUITY_START_SHA256" = "$PROCESS_CONTINUITY_END_SHA256" ]
then
    PROCESS_CONTINUITY_FINGERPRINT_MATCH_GATE="PASS"
    PROCESS_CONTINUITY_GATE="PASS"
else
    PROCESS_CONTINUITY_FINGERPRINT_MATCH_GATE="FAIL"
    PROCESS_CONTINUITY_GATE="FAIL"
fi

echo "PROCESS_CONTINUITY_FINGERPRINT_MATCH_GATE=${PROCESS_CONTINUITY_FINGERPRINT_MATCH_GATE}"
echo "PROCESS_CONTINUITY_CONTAINER_SET=5GC,RIC,GNB,UE"
echo "PROCESS_CONTINUITY_IDENTITY_FIELDS=NAME,ID,PID,STARTED_AT,RESTART_COUNT,IMAGE_ID"
echo "PROCESS_CONTINUITY_STATE_FIELDS=RUNNING,STATUS,HEALTH"
echo "PROCESS_CONTINUITY_RESTART_COUNT_ZERO_REQUIRED=NO"
echo "PROCESS_CONTINUITY_RESTART_COUNT_STABILITY_REQUIRED=YES"
echo "PROCESS_CONTINUITY_START_END_MATCH_REQUIRED=YES"
echo "PROCESS_CONTINUITY_GATE=${PROCESS_CONTINUITY_GATE}"

echo
echo "===== FUTURE READINESS GATES ====="

READINESS_FRESHNESS_GATE="PENDING"
...
elif [ "$PROCESS_CONTINUITY_GATE" != "PASS" ]; then
    gate="FAIL"
    failure_reason="PROCESS_CONTINUITY_GATE"
else
    gate="FAIL"
...
echo
echo "===== READINESS ARTIFACT ====="

READINESS_ARTIFACT_SCHEMA_VERSION="1"
READINESS_ARTIFACT_ROOT="/tmp/sci-oran/readiness"
READINESS_ARTIFACT_RUN_DIR="${READINESS_ARTIFACT_ROOT}/${READINESS_ID}"
READINESS_ARTIFACT_RUN_PATH="${READINESS_ARTIFACT_RUN_DIR}/readiness.env"
READINESS_ARTIFACT_LATEST_PATH="${READINESS_ARTIFACT_ROOT}/latest.env"

READINESS_ARTIFACT_TMP_PATH="${READINESS_ARTIFACT_RUN_DIR}/.readiness.env.tmp.$$"
READINESS_ARTIFACT_LATEST_TMP_PATH="${READINESS_ARTIFACT_ROOT}/.latest.env.tmp.$$"

READINESS_ARTIFACT_ROOT_GATE="FAIL"
READINESS_ARTIFACT_BUILD_GATE="FAIL"
READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE="FAIL"
READINESS_ARTIFACT_LATEST_COPY_GATE="FAIL"
READINESS_ARTIFACT_VALIDATION_GATE="FAIL"
READINESS_ARTIFACT_LATEST_MATCH_GATE="FAIL"
READINESS_ARTIFACT_GATE="FAIL"

READINESS_ARTIFACT_GENERATED_AT_UTC="$(
    date -u '+%Y-%m-%dT%H:%M:%SZ'
)"

READINESS_ARTIFACT_HOSTNAME="$(hostname)"

READINESS_ARTIFACT_GIT_BRANCH="$(
    git branch --show-current 2>/dev/null ||
    true
)"

READINESS_ARTIFACT_GIT_HEAD="$(
    git rev-parse HEAD 2>/dev/null ||
    true
)"

if [ -n "$(git status --porcelain 2>/dev/null || true)" ]; then
    READINESS_ARTIFACT_GIT_WORKTREE="DIRTY"
else
    READINESS_ARTIFACT_GIT_WORKTREE="CLEAN"
fi

echo "READINESS_ARTIFACT_SCHEMA_VERSION=${READINESS_ARTIFACT_SCHEMA_VERSION}"
echo "READINESS_ARTIFACT_ROOT=${READINESS_ARTIFACT_ROOT}"
echo "READINESS_ARTIFACT_RUN_DIR=${READINESS_ARTIFACT_RUN_DIR}"
echo "READINESS_ARTIFACT_RUN_PATH=${READINESS_ARTIFACT_RUN_PATH}"
echo "READINESS_ARTIFACT_LATEST_PATH=${READINESS_ARTIFACT_LATEST_PATH}"
echo "READINESS_ARTIFACT_GENERATED_AT_UTC=${READINESS_ARTIFACT_GENERATED_AT_UTC}"

if mkdir -p "$READINESS_ARTIFACT_RUN_DIR"; then
    READINESS_ARTIFACT_ROOT_GATE="PASS"
fi

echo "READINESS_ARTIFACT_ROOT_GATE=${READINESS_ARTIFACT_ROOT_GATE}"

if [ "$READINESS_ARTIFACT_ROOT_GATE" = "PASS" ]; then

    if cat > "$READINESS_ARTIFACT_TMP_PATH" <<EOF
ARTIFACT_SCHEMA_VERSION=1
READINESS_ID=${READINESS_ID}
GENERATED_AT_UTC=${READINESS_ARTIFACT_GENERATED_AT_UTC}
DOCTOR_VERSION=${DOCTOR_VERSION}
HOSTNAME=${READINESS_ARTIFACT_HOSTNAME}
REPO_ROOT=${REPO_ROOT}
GIT_BRANCH=${READINESS_ARTIFACT_GIT_BRANCH}
GIT_HEAD=${READINESS_ARTIFACT_GIT_HEAD}
GIT_WORKTREE=${READINESS_ARTIFACT_GIT_WORKTREE}
PROCESS_CONTINUITY_START_SHA256=${PROCESS_CONTINUITY_START_SHA256}
PROCESS_CONTINUITY_END_SHA256=${PROCESS_CONTINUITY_END_SHA256}
PROCESS_CONTINUITY_GATE=${PROCESS_CONTINUITY_GATE}
USER_PLANE_EVIDENCE_TIMESTAMP=${USER_PLANE_EVIDENCE_TIMESTAMP}
USER_PLANE_EVIDENCE_AGE_SECONDS=${USER_PLANE_EVIDENCE_AGE_SECONDS}
DOCTOR_READ_ONLY_GATE=PASS
HOST_READINESS_GATE=${HOST_READINESS_GATE}
HOST_RESOURCE_READINESS_GATE=${HOST_RESOURCE_READINESS_GATE}
CONTAINER_READINESS_GATE=${CONTAINER_READINESS_GATE}
NETWORK_READINESS_GATE=${NETWORK_READINESS_GATE}
ZMQ_READINESS_GATE=${ZMQ_READINESS_GATE}
N2_READINESS_GATE=${N2_READINESS_GATE}
E2_READINESS_GATE=${E2_READINESS_GATE}
E2SM_RC_READINESS_GATE=${E2SM_RC_READINESS_GATE}
RAN_FUNCTION_3_GATE=${RAN_FUNCTION_3_GATE:-MISSING}
UE_SESSION_READINESS_GATE=${UE_SESSION_READINESS_GATE}
USER_PLANE_READINESS_GATE=${USER_PLANE_READINESS_GATE}
TRAFFIC_HARNESS_READINESS_GATE=${TRAFFIC_HARNESS_READINESS_GATE}
REPOSITORY_REPRODUCIBILITY_GATE=${REPOSITORY_REPRODUCIBILITY_GATE}
SCI_ORAN_READY_GATE=${gate}
FAILURE_REASON=${failure_reason}
EOF
    then
        READINESS_ARTIFACT_BUILD_GATE="PASS"
    fi
fi

echo "READINESS_ARTIFACT_BUILD_GATE=${READINESS_ARTIFACT_BUILD_GATE}"

if [ "$READINESS_ARTIFACT_BUILD_GATE" = "PASS" ]; then
    if mv \
        "$READINESS_ARTIFACT_TMP_PATH" \
        "$READINESS_ARTIFACT_RUN_PATH"
    then
        READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE="PASS"
    fi
fi

echo "READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE=${READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE}"

if [ "$READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE" = "PASS" ]; then
    if cp \
        "$READINESS_ARTIFACT_RUN_PATH" \
        "$READINESS_ARTIFACT_LATEST_TMP_PATH" &&
       mv \
        "$READINESS_ARTIFACT_LATEST_TMP_PATH" \
        "$READINESS_ARTIFACT_LATEST_PATH"
    then
        READINESS_ARTIFACT_LATEST_COPY_GATE="PASS"
    fi
fi

echo "READINESS_ARTIFACT_LATEST_COPY_GATE=${READINESS_ARTIFACT_LATEST_COPY_GATE}"

if [ "$READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE" = "PASS" ]; then

    if python3 - "$READINESS_ARTIFACT_RUN_PATH" <<'PYART'
from pathlib import Path
import sys
...
required = {
    ...
    "USER_PLANE_EVIDENCE_TIMESTAMP",
    "USER_PLANE_EVIDENCE_AGE_SECONDS",
    "DOCTOR_READ_ONLY_GATE",
    "HOST_READINESS_GATE",
...
if data["ARTIFACT_SCHEMA_VERSION"] != "1":
    raise SystemExit(1)

if data["PROCESS_CONTINUITY_START_SHA256"] != \
        data["PROCESS_CONTINUITY_END_SHA256"]:
    raise SystemExit(1)

if data["PROCESS_CONTINUITY_GATE"] != "PASS":
    raise SystemExit(1)

if "READINESS_ARTIFACT_GATE" in data:
    raise SystemExit(1)

if "READINESS_FRESHNESS_GATE" in data:
    raise SystemExit(1)
...
PYART
    then
        READINESS_ARTIFACT_VALIDATION_GATE="PASS"
    fi
fi

echo "READINESS_ARTIFACT_VALIDATION_GATE=${READINESS_ARTIFACT_VALIDATION_GATE}"

if [ "$READINESS_ARTIFACT_VALIDATION_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_LATEST_COPY_GATE" = "PASS" ] &&
   cmp -s \
       "$READINESS_ARTIFACT_RUN_PATH" \
       "$READINESS_ARTIFACT_LATEST_PATH"
then
    READINESS_ARTIFACT_LATEST_MATCH_GATE="PASS"
fi

echo "READINESS_ARTIFACT_LATEST_MATCH_GATE=${READINESS_ARTIFACT_LATEST_MATCH_GATE}"

if [ "$READINESS_ARTIFACT_ROOT_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_BUILD_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_ATOMIC_PUBLISH_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_LATEST_COPY_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_VALIDATION_GATE" = "PASS" ] &&
...
if [ -f "$READINESS_ARTIFACT_RUN_PATH" ]; then
    READINESS_ARTIFACT_SHA256="$(
        sha256sum "$READINESS_ARTIFACT_RUN_PATH" |
        awk '{print $1}'
    )"
else
    READINESS_ARTIFACT_SHA256="MISSING"
fi

echo "READINESS_ARTIFACT_SHA256=${READINESS_ARTIFACT_SHA256}"
echo "READINESS_ARTIFACT_GATE=${READINESS_ARTIFACT_GATE}"

if [ "$READINESS_ARTIFACT_GATE" != "PASS" ]; then
    gate="FAIL"

    if [ "$failure_reason" = "MANDATORY_GATES_NOT_YET_IMPLEMENTED" ]; then
        failure_reason="READINESS_ARTIFACT_GATE"
    fi
fi

echo
echo "===== READINESS FRESHNESS ====="

READINESS_FRESHNESS_SCHEMA_VERSION="1"

READINESS_RUN_MAX_DURATION_SECONDS="120"
USER_PLANE_MAX_AGE_AT_ARTIFACT_SECONDS="300"
USER_PLANE_AGE_DRIFT_MAX_SECONDS="30"
CLOCK_FUTURE_SKEW_MAX_SECONDS="5"
FINAL_DECISION_AFTER_ARTIFACT_MAX_SECONDS="60"

READINESS_FRESHNESS_ID_COHERENCE_GATE="FAIL"
READINESS_FRESHNESS_TIMESTAMP_PARSE_GATE="FAIL"
READINESS_FRESHNESS_TEMPORAL_ORDER_GATE="FAIL"
READINESS_FRESHNESS_RUN_DURATION_GATE="FAIL"
READINESS_FRESHNESS_USER_PLANE_AGE_GATE="FAIL"
READINESS_FRESHNESS_USER_PLANE_AGE_COHERENCE_GATE="FAIL"
READINESS_FRESHNESS_PROCESS_CONTINUITY_GATE="FAIL"
READINESS_FRESHNESS_ARTIFACT_GATE="FAIL"
READINESS_FRESHNESS_FINAL_DECISION_LATENCY_GATE="FAIL"
READINESS_FRESHNESS_GATE="FAIL"

read_artifact_value()
{
    key="$1"

    grep "^${key}=" "$READINESS_ARTIFACT_RUN_PATH" \
        2>/dev/null |
    tail -n 1 |
    cut -d= -f2-
}

READINESS_RUN_START_UTC="${UTC_TIMESTAMP:-}"

ARTIFACT_READINESS_ID="$(
    read_artifact_value READINESS_ID
)"

ARTIFACT_GENERATED_AT_UTC="$(
    read_artifact_value GENERATED_AT_UTC
)"

ARTIFACT_USER_PLANE_TIMESTAMP="$(
    read_artifact_value USER_PLANE_EVIDENCE_TIMESTAMP
)"

ARTIFACT_USER_PLANE_STORED_AGE="$(
    read_artifact_value USER_PLANE_EVIDENCE_AGE_SECONDS
)"

echo "READINESS_FRESHNESS_SCHEMA_VERSION=${READINESS_FRESHNESS_SCHEMA_VERSION}"
echo "READINESS_RUN_START_UTC=${READINESS_RUN_START_UTC:-MISSING}"
echo "READINESS_ARTIFACT_TIME_UTC=${ARTIFACT_GENERATED_AT_UTC:-MISSING}"
echo "READINESS_USER_PLANE_TIME_UTC=${ARTIFACT_USER_PLANE_TIMESTAMP:-MISSING}"
echo "READINESS_USER_PLANE_STORED_AGE_SECONDS=${ARTIFACT_USER_PLANE_STORED_AGE:-MISSING}"

if [ -n "$READINESS_ID" ] &&
   [ "$READINESS_ID" = "$ARTIFACT_READINESS_ID" ]
then
    READINESS_FRESHNESS_ID_COHERENCE_GATE="PASS"
fi

echo "READINESS_FRESHNESS_ID_COHERENCE_GATE=${READINESS_FRESHNESS_ID_COHERENCE_GATE}"

RUN_START_EPOCH=""
ARTIFACT_EPOCH=""
UP_EPOCH=""

if [ -n "$READINESS_RUN_START_UTC" ] &&
   [ -n "$ARTIFACT_GENERATED_AT_UTC" ] &&
   [ -n "$ARTIFACT_USER_PLANE_TIMESTAMP" ]
then
    RUN_START_EPOCH="$(
        date -u -d "$READINESS_RUN_START_UTC" +%s \
            2>/dev/null ||
        true
    )"

    ARTIFACT_EPOCH="$(
        date -u -d "$ARTIFACT_GENERATED_AT_UTC" +%s \
            2>/dev/null ||
        true
    )"

    UP_EPOCH="$(
        date -u -d "$ARTIFACT_USER_PLANE_TIMESTAMP" +%s \
            2>/dev/null ||
        true
    )"
fi

if [[ "$RUN_START_EPOCH" =~ ^[0-9]+$ ]] &&
   [[ "$ARTIFACT_EPOCH" =~ ^[0-9]+$ ]] &&
   [[ "$UP_EPOCH" =~ ^[0-9]+$ ]] &&
   [[ "$ARTIFACT_USER_PLANE_STORED_AGE" =~ ^[0-9]+$ ]]
then
    READINESS_FRESHNESS_TIMESTAMP_PARSE_GATE="PASS"
fi

echo "READINESS_FRESHNESS_TIMESTAMP_PARSE_GATE=${READINESS_FRESHNESS_TIMESTAMP_PARSE_GATE}"

READINESS_RUN_DURATION_SECONDS="MISSING"
USER_PLANE_EFFECTIVE_AGE_SECONDS="MISSING"
USER_PLANE_AGE_DRIFT_SECONDS="MISSING"

if [ "$READINESS_FRESHNESS_TIMESTAMP_PARSE_GATE" = "PASS" ]; then

    READINESS_RUN_DURATION_SECONDS="$(
        printf '%s\n' "$((ARTIFACT_EPOCH - RUN_START_EPOCH))"
    )"

    USER_PLANE_EFFECTIVE_AGE_SECONDS="$(
        printf '%s\n' "$((ARTIFACT_EPOCH - UP_EPOCH))"
    )"

    USER_PLANE_AGE_DRIFT_SECONDS="$(
        printf '%s\n' "$((USER_PLANE_EFFECTIVE_AGE_SECONDS - ARTIFACT_USER_PLANE_STORED_AGE))"
    )"

    if [ "$USER_PLANE_AGE_DRIFT_SECONDS" -lt 0 ]; then
        USER_PLANE_AGE_DRIFT_SECONDS="$(
            printf '%s\n' "$((-USER_PLANE_AGE_DRIFT_SECONDS))"
        )"
    fi

    if [ "$READINESS_RUN_DURATION_SECONDS" -ge \
         "-$CLOCK_FUTURE_SKEW_MAX_SECONDS" ] &&
       [ "$USER_PLANE_EFFECTIVE_AGE_SECONDS" -ge \
         "-$CLOCK_FUTURE_SKEW_MAX_SECONDS" ]
    then
        READINESS_FRESHNESS_TEMPORAL_ORDER_GATE="PASS"
    fi

    EFFECTIVE_RUN_DURATION="$READINESS_RUN_DURATION_SECONDS"

    if [ "$EFFECTIVE_RUN_DURATION" -lt 0 ]; then
        EFFECTIVE_RUN_DURATION="0"
    fi

    EFFECTIVE_UP_AGE="$USER_PLANE_EFFECTIVE_AGE_SECONDS"

    if [ "$EFFECTIVE_UP_AGE" -lt 0 ]; then
        EFFECTIVE_UP_AGE="0"
    fi

    if [ "$EFFECTIVE_RUN_DURATION" -le \
         "$READINESS_RUN_MAX_DURATION_SECONDS" ]
    then
        READINESS_FRESHNESS_RUN_DURATION_GATE="PASS"
    fi

    if [ "$EFFECTIVE_UP_AGE" -le \
         "$USER_PLANE_MAX_AGE_AT_ARTIFACT_SECONDS" ]
    then
        READINESS_FRESHNESS_USER_PLANE_AGE_GATE="PASS"
    fi

    if [ "$USER_PLANE_AGE_DRIFT_SECONDS" -le \
         "$USER_PLANE_AGE_DRIFT_MAX_SECONDS" ]
    then
        READINESS_FRESHNESS_USER_PLANE_AGE_COHERENCE_GATE="PASS"
    fi
fi

echo "READINESS_RUN_DURATION_SECONDS=${READINESS_RUN_DURATION_SECONDS}"
echo "USER_PLANE_EFFECTIVE_AGE_SECONDS=${USER_PLANE_EFFECTIVE_AGE_SECONDS}"
echo "USER_PLANE_AGE_DRIFT_SECONDS=${USER_PLANE_AGE_DRIFT_SECONDS}"

echo "READINESS_FRESHNESS_TEMPORAL_ORDER_GATE=${READINESS_FRESHNESS_TEMPORAL_ORDER_GATE}"
echo "READINESS_FRESHNESS_RUN_DURATION_GATE=${READINESS_FRESHNESS_RUN_DURATION_GATE}"
echo "READINESS_FRESHNESS_USER_PLANE_AGE_GATE=${READINESS_FRESHNESS_USER_PLANE_AGE_GATE}"
echo "READINESS_FRESHNESS_USER_PLANE_AGE_COHERENCE_GATE=${READINESS_FRESHNESS_USER_PLANE_AGE_COHERENCE_GATE}"

if [ "$PROCESS_CONTINUITY_GATE" = "PASS" ]; then
    READINESS_FRESHNESS_PROCESS_CONTINUITY_GATE="PASS"
fi

if [ "$READINESS_ARTIFACT_GATE" = "PASS" ]; then
    READINESS_FRESHNESS_ARTIFACT_GATE="PASS"
fi

echo "READINESS_FRESHNESS_PROCESS_CONTINUITY_GATE=${READINESS_FRESHNESS_PROCESS_CONTINUITY_GATE}"
echo "READINESS_FRESHNESS_ARTIFACT_GATE=${READINESS_FRESHNESS_ARTIFACT_GATE}"

READINESS_FINAL_DECISION_AT_UTC="$(
    date -u '+%Y-%m-%dT%H:%M:%SZ'
)"

READINESS_FINAL_DECISION_EPOCH="$(
    date -u -d "$READINESS_FINAL_DECISION_AT_UTC" +%s
)"

FINAL_DECISION_AFTER_ARTIFACT_SECONDS="MISSING"

if [[ "$ARTIFACT_EPOCH" =~ ^[0-9]+$ ]]; then

    FINAL_DECISION_AFTER_ARTIFACT_SECONDS="$(
        printf '%s\n' "$((READINESS_FINAL_DECISION_EPOCH - ARTIFACT_EPOCH))"
    )"

    if [ "$FINAL_DECISION_AFTER_ARTIFACT_SECONDS" -ge \
         "-$CLOCK_FUTURE_SKEW_MAX_SECONDS" ] &&
       [ "$FINAL_DECISION_AFTER_ARTIFACT_SECONDS" -le \
         "$FINAL_DECISION_AFTER_ARTIFACT_MAX_SECONDS" ]
    then
        READINESS_FRESHNESS_FINAL_DECISION_LATENCY_GATE="PASS"
    fi
fi

echo "READINESS_FINAL_DECISION_AT_UTC=${READINESS_FINAL_DECISION_AT_UTC}"
echo "FINAL_DECISION_AFTER_ARTIFACT_SECONDS=${FINAL_DECISION_AFTER_ARTIFACT_SECONDS}"
echo "READINESS_FRESHNESS_FINAL_DECISION_LATENCY_GATE=${READINESS_FRESHNESS_FINAL_DECISION_LATENCY_GATE}"

if [ "$READINESS_FRESHNESS_ID_COHERENCE_GATE" = "PASS" ] &&
   [ "$READINESS_FRESHNESS_TIMESTAMP_PARSE_GATE" = "PASS" ] &&
   [ "$READINESS_FRESHNESS_TEMPORAL_ORDER_GATE" = "PASS" ] &&
   [ "$READINESS_FRESHNESS_RUN_DURATION_GATE" = "PASS" ] &&
   [ "$READINESS_FRESHNESS_USER_PLANE_AGE_GATE" = "PASS" ] &&
...
echo "READINESS_FRESHNESS_GATE=${READINESS_FRESHNESS_GATE}"

echo
echo "===== FINAL READY DECISION ====="

if [ "$READINESS_ARTIFACT_GATE" = "PASS" ] &&
   [ "$READINESS_FRESHNESS_GATE" = "PASS" ]
then
    gate="PASS"
    failure_reason="NONE"

elif [ "$READINESS_ARTIFACT_GATE" != "PASS" ]; then
    gate="FAIL"
    ...
    gate="FAIL"
    failure_reason="READINESS_FRESHNESS_GATE"
fi

echo "FINAL_PRE_ARTIFACT_SCI_ORAN_READY_GATE=${gate}"
echo "FINAL_PRE_ARTIFACT_FAILURE_REASON=${failure_reason}"

echo
echo "===== READINESS ARTIFACT FINALIZATION ====="

READINESS_ARTIFACT_FINALIZATION_BUILD_GATE="FAIL"
READINESS_ARTIFACT_FINALIZATION_PUBLISH_GATE="FAIL"
READINESS_ARTIFACT_FINALIZATION_LATEST_GATE="FAIL"
READINESS_ARTIFACT_FINALIZATION_MATCH_GATE="FAIL"
READINESS_ARTIFACT_FINALIZATION_VALUE_GATE="FAIL"
READINESS_ARTIFACT_FINALIZATION_GATE="FAIL"

READINESS_ARTIFACT_FINAL_TMP="${READINESS_ARTIFACT_RUN_DIR}/.readiness.final.env.tmp.$$"
READINESS_ARTIFACT_FINAL_LATEST_TMP="${READINESS_ARTIFACT_ROOT}/.latest.final.env.tmp.$$"

if [ -f "$READINESS_ARTIFACT_RUN_PATH" ]; then

    if python3 - \
        "$READINESS_ARTIFACT_RUN_PATH" \
        "$READINESS_ARTIFACT_FINAL_TMP" \
        "$gate" \
        "$failure_reason" <<'PYFINAL'
from pathlib import Path
import sys
...
for line in lines:
    if line.startswith("SCI_ORAN_READY_GATE="):
        out.append(f"SCI_ORAN_READY_GATE={ready}")
        ready_count += 1

    elif line.startswith("FAILURE_REASON="):
        out.append(f"FAILURE_REASON={reason}")
        reason_count += 1
...
PYFINAL
    then
        READINESS_ARTIFACT_FINALIZATION_BUILD_GATE="PASS"
    fi
fi

echo "READINESS_ARTIFACT_FINALIZATION_BUILD_GATE=${READINESS_ARTIFACT_FINALIZATION_BUILD_GATE}"

if [ "$READINESS_ARTIFACT_FINALIZATION_BUILD_GATE" = "PASS" ]; then

    if cp \
        "$READINESS_ARTIFACT_FINAL_TMP" \
        "$READINESS_ARTIFACT_FINAL_LATEST_TMP"
    then
        if mv \
            "$READINESS_ARTIFACT_FINAL_TMP" \
            "$READINESS_ARTIFACT_RUN_PATH"
        then
            READINESS_ARTIFACT_FINALIZATION_PUBLISH_GATE="PASS"
        fi
        ...
        if mv \
            ...
            "$READINESS_ARTIFACT_LATEST_PATH"
        then
            READINESS_ARTIFACT_FINALIZATION_LATEST_GATE="PASS"
        fi
    fi
fi

echo "READINESS_ARTIFACT_FINALIZATION_PUBLISH_GATE=${READINESS_ARTIFACT_FINALIZATION_PUBLISH_GATE}"
echo "READINESS_ARTIFACT_FINALIZATION_LATEST_GATE=${READINESS_ARTIFACT_FINALIZATION_LATEST_GATE}"

if [ "$READINESS_ARTIFACT_FINALIZATION_PUBLISH_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_FINALIZATION_LATEST_GATE" = "PASS" ] &&
   cmp -s \
       "$READINESS_ARTIFACT_RUN_PATH" \
       "$READINESS_ARTIFACT_LATEST_PATH"
then
    READINESS_ARTIFACT_FINALIZATION_MATCH_GATE="PASS"
fi

echo "READINESS_ARTIFACT_FINALIZATION_MATCH_GATE=${READINESS_ARTIFACT_FINALIZATION_MATCH_GATE}"

FINAL_ARTIFACT_READY="$(
    read_artifact_value SCI_ORAN_READY_GATE
)"

FINAL_ARTIFACT_REASON="$(
    read_artifact_value FAILURE_REASON
)"

echo "FINAL_ARTIFACT_SCI_ORAN_READY_GATE=${FINAL_ARTIFACT_READY:-MISSING}"
echo "FINAL_ARTIFACT_FAILURE_REASON=${FINAL_ARTIFACT_REASON:-MISSING}"

if [ "$FINAL_ARTIFACT_READY" = "$gate" ] &&
   [ "$FINAL_ARTIFACT_REASON" = "$failure_reason" ]
then
    READINESS_ARTIFACT_FINALIZATION_VALUE_GATE="PASS"
fi

echo "READINESS_ARTIFACT_FINALIZATION_VALUE_GATE=${READINESS_ARTIFACT_FINALIZATION_VALUE_GATE}"

if [ "$READINESS_ARTIFACT_FINALIZATION_BUILD_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_FINALIZATION_PUBLISH_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_FINALIZATION_LATEST_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_FINALIZATION_MATCH_GATE" = "PASS" ] &&
   [ "$READINESS_ARTIFACT_FINALIZATION_VALUE_GATE" = "PASS" ]
...
echo "READINESS_ARTIFACT_FINALIZATION_GATE=${READINESS_ARTIFACT_FINALIZATION_GATE}"

if [ "$READINESS_ARTIFACT_FINALIZATION_GATE" != "PASS" ]; then
    READINESS_ARTIFACT_GATE="FAIL"
    gate="FAIL"
    failure_reason="READINESS_ARTIFACT_FINALIZATION_GATE"
fi
...
echo "FINAL_READINESS_ARTIFACT_SHA256=${READINESS_ARTIFACT_SHA256}"
echo "READINESS_ARTIFACT_GATE=${READINESS_ARTIFACT_GATE}"
